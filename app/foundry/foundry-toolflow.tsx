  ...
  return <section className="foundry-toolflow" id="toolflow" aria-labelledby="toolflow-heading">
    ...
    <ol className="foundry-toolflow-track" aria-label="WebMCP tool lifecycle">
      {stages.map((stage, index) => <li key={stage.id} data-state={stage.state}>
        ...
        <button className="foundry-toolflow-node" type="button" aria-pressed={stage.id === selected.id} onClick={() => setSelectedId(stage.id)}><span>{stage.state === "COMPLETE" ? "✓" : stage.state === "BLOCKED" ? "×" : stage.state === "ACTIVE" ? "◉" : "○"}</span><b>{stage.label}</b><small>{toolflowStateLabel(stage.state)}</small></button>
        <p>{stage.detail}</p>
      </li>)}
    </ol>
    <section className="foundry-toolflow-inspector" aria-live="polite"><div><span className="foundry-state">INSPECTING · {selected.label.toUpperCase()}</span><h3>{toolflowStateLabel(selected.state)}</h3><p>{selected.detail}</p></div>{selected.events.length ? <ol>{selected.events.map((event, index) => <li key={`${event.type}-${index}`} data-status={event.status}><b>{event.label}</b><span>{event.detail}</span></li>)}</ol> : <p className="foundry-empty">This node has no emitted activity to inspect yet.</p>}</section>

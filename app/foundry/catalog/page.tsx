...
export default function FoundryCatalogPage() {
  ...
  return <main className="foundry foundry-catalog-page">
    <header className="foundry-top"><Link href="/">XACT</Link><span>FOUNDRY CATALOG</span><strong>Approved recipes are not tools until Xact constructs and verifies them.</strong></header>
    ...
    <section className="foundry-profile-panel"><div><span className="foundry-state">PROFILE CONTROLS · SESSION ONLY</span><h2>How should Foundry prepare work?</h2><p>These are explicit build preferences, not hidden model training. Campaign voice and style are applied only to draft preparation; delivery stays draft-only.</p></div><label className="foundry-label">Prioritize<select value={profileDraft.focus} onChange={(event) => setProfileDraft((current) => ({ ...current, focus: event.target.value as FoundryProfile["focus"] }))}><option value="CUSTOMER_OPERATIONS">Customer and operations</option><option value="CAMPAIGN_OPERATIONS">Campaigns and content</option></select></label><label className="foundry-label">Brand voice<input value={profileDraft.brandVoice} onChange={(event) => setProfileDraft((current) => ({ ...current, brandVoice: event.target.value }))} /></label><label className="foundry-label">Campaign style<input value={profileDraft.campaignStyle} onChange={(event) => setProfileDraft((current) => ({ ...current, campaignStyle: event.target.value }))} /></label><label className="foundry-label">Promotion offer<input value={profileDraft.campaignOffer} onChange={(event) => setProfileDraft((current) => ({ ...current, campaignOffer: event.target.value }))} /></label><label className="foundry-label">Default actor<input value={profileDraft.defaultActor} onChange={(event) => setProfileDraft((current) => ({ ...current, defaultActor: event.target.value }))} /></label><label className="foundry-label">Service-credit ceiling<input type="number" value={profileDraft.serviceCreditCeiling} onChange={(event) => setProfileDraft((current) => ({ ...current, serviceCreditCeiling: event.target.value }))} /></label><button className="foundry-build foundry-profile-apply" type="button" onClick={applyProfile}>APPLY PROFILE REVISION</button><p className="foundry-profile-lock">APPROVED AUDIENCE · Foundry mock customer directory<br />DELIVERY MODE · DRAFT ONLY<br />{tools.length} governed tool variant{tools.length === 1 ? "" : "s"} constructed in this session</p></section>

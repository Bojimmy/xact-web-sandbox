...
test("public-safe business workspace exposes deterministic task-shaped read results", () => {
  const results = [readWorkOrderQueue(), readSupportQueue(), readEmployeeDirectory(), readCustomerHealth("1042"), readOperationsReport(), readCampaignDashboard(), readSalesPipeline(), readMarketingPerformance()];
  assert.deepEqual(results.map((result) => result.kind), ["WORK_ORDER_QUEUE", "SUPPORT_QUEUE", "EMPLOYEE_DIRECTORY", "CUSTOMER_HEALTH", "OPERATIONS_REPORT", "CAMPAIGN_DASHBOARD", "SALES_PIPELINE", "MARKETING_PERFORMANCE"]);
  ...
  assert.equal(readSalesPipeline().summary.find((item) => item.label === "Open pipeline")?.value, "$96,300");
  assert.equal(readMarketingPerformance().summary.find((item) => item.label === "Delivery")?.value, "0");
});

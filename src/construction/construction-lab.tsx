...
import type { ConstructionRun, Product } from "./contracts";
...
export function ConstructionLab() {
  ...
  const benchmark = async () => {
    setBusy(true);
    try { setComparison(await Promise.all(levels.map((concurrency) => engine.run({ request: inventoryBenchmarkRequest, concurrency })))); }
    finally { setBusy(false); }
  };
  const advance = () => {

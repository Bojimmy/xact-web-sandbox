...
function ServiceOperationsConsole({ artifact }: { artifact: ServiceOperationsConsoleArtifact }) {
  ...
  return <section className="construction-artifact service-operations-console" aria-labelledby="service-operations-title">
    ...
    <div className="service-console-grid"><section><h4>Available actions</h4><ul>{customer.availableActions.map((action) => <li key={action}>{action}</li>)}</ul><button type="button" data-xact-target={`customer:${customer.id}/service-credit`} disabled aria-disabled="true">Apply service credit · Commit required</button><button type="button" disabled aria-disabled="true">Change service plan · Commit required</button></section><section><h4>Audit history</h4><ol>{auditHistory.map((event) => <li key={event.id}><span>{event.detail}</span><time>{event.recordedAt}</time></li>)}</ol></section></div>
